...
//            System.out.println("Top Start:"+new Date());
            if (repType.equalsIgnoreCase("Customer Id")) {
                ...
                for (int i = 0; i < reportList.size(); i++) {
                    ...
                    String coBorrower = "";
                    List jointList = em.createNativeQuery("select b.acno,b.custname,b.joinDetails from accountmaster a,(select a.acno, a.custname, "
                            ...
                            + "where (a.custid1<> null or a.custid1<>'') or (a.custid2<> null or a.custid2<>'') or "
                            + "(a.custid3<> null or a.custid3<>'') or (a.custid4<> null or a.custid4<>''))b "
                            + "where a.acno = b.acno and a.acno = '" + val.getAcNo() + "'").getResultList();

                    if (!jointList.isEmpty()) {
                        Vector vtr = (Vector) jointList.get(0);
                        coBorrower = vtr.get(2).toString();
                    }
                    pojo.setCoBorrower(coBorrower);
