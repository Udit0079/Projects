 ...
 * @author root
 */
public class ClearingScroll extends BaseBean{
    ...
    public ClearingScroll() {
        try {
            ...
            dropDown1List.add(new SelectItem("CLG", "CLEARING SCROLL REPORT"));
            dropDown1List.add(new SelectItem("CLG", "INWARD CLEARING"));
    ...
    public void btnHtmlAction() {
        this.setMessage("");
        String report = "Clearing Scroll Report";
        ...
        if (calDate.equals(getTodayDate())) {
            //    Date datee = this.calDate;
            String date2 = calDate;
            String user = getUserName();
            Map fillParams = new HashMap();
            try {
                String dt2[] = date.split("/");
                String frmDate = dt2[2] + "-" + dt2[1] + "-" + dt2[0];
                String fromTime = "";
                ...
                cashScrollDataList = ledgerReportRemote.getCbsCashClgScrollData(indate, dropDown1, this.getBranchOption(), fromTime, toTime, timeAllowed,orderBy);
                if (!cashScrollDataList.isEmpty()) {
                    new ReportBean().jasperReport("clg_scroll", "text/html",
                            new JRBeanCollectionDataSource(cashScrollDataList), fillParams, "Clearing Scroll Report");
            ...
            try {
                ...
                String indate = dt1[2] + dt1[1] + dt1[0];
                //pass Parameter "A" For Bifercation As in Transfer Scroll Hard Code "A" For ALL
                cashScrollTimewiseDataList = ledgerReportRemote.getCbsCashTrfClgScrollData(indate, dropDown1, this.getBranchOption(), fromTime, toTime, timeAllowed,orderBy,"A");
                if (!cashScrollTimewiseDataList.isEmpty()) {
                    new ReportBean().jasperReport("clg_scroll_1", "text/html",
                            new JRBeanCollectionDataSource(cashScrollTimewiseDataList), fillParams, "Clearing Scroll Report");
                    ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
                    HttpSession httpSession = getHttpSession();
                    httpSession.setAttribute("ReportName", report);
                } else {
       ...
       // String dateString = sdf.format(date);
        if (calDate.equals(getTodayDate())) {
            ...
            try {
                ...
                String indate = dt2[2] + dt2[1] + dt2[0];
                cashScrollDataList = ledgerReportRemote.getCbsCashClgScrollData(indate, dropDown1, this.getBranchOption(), printTime1, printTime2, timeAllowed,orderBy);
                
//                new ReportBean().jasperReport("clg_scroll", "application/pdf",
//                        new JRBeanCollectionDataSource(cashScrollDataList), fillParams, "Clearing Scroll Report");
//                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
//                HttpSession httpSession = getHttpSession();
//                httpSession.setAttribute("ReportName", report);
                
                 new ReportBean().openPdf("clg_scroll_" , "clg_scroll", new JRBeanCollectionDataSource(cashScrollDataList), fillParams);  
                 ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
            HttpSession httpSession = getHttpSession();
            httpSession.setAttribute("ReportName", report);
            } catch (Exception e) {
            ...
            try {
                ...
                String indate = dt1[2] + dt1[1] + dt1[0];
                //pass Parameter "A" For Bifercation As in Transfer Scroll Hard Code "A" For ALL
                cashScrollTimewiseDataList = ledgerReportRemote.getCbsCashTrfClgScrollData(indate,
                        dropDown1, this.getBranchOption(), fromTime, toTime, timeAllowed,orderBy,"A");
                
//                new ReportBean().jasperReport("clg_scroll_1", "application/pdf",
//                        new JRBeanCollectionDataSource(cashScrollTimewiseDataList), fillParams, "Clearing Scroll Report");
//                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
//                HttpSession httpSession = getHttpSession();
//                httpSession.setAttribute("ReportName", report);
                
                new ReportBean().openPdf("clg_scroll_1_" , "clg_scroll_1", new JRBeanCollectionDataSource(cashScrollTimewiseDataList), fillParams);  
                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
            HttpSession httpSession = getHttpSession();
            httpSession.setAttribute("ReportName", report);
            } catch (Exception e) {
