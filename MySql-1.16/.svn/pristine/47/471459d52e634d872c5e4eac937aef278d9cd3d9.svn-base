 ...
 * @author root
 */
public final class MinBalanceCharges {
    ...
    public void accountToCredit() {
        ...
        try {
            List resultList = new ArrayList();
            resultList = remoteObject.setAcToCredit(acctType);
    ...
    public void postBtnAction() {
        try {
            ...
            String result = remoteObject.minBalanceChargesPost(resultList, acctType, glAccountNo, userName, orgnBrCode, creditAmount, currentDate1, ymd.format(fromDate), ymd.format(toDate));
            if (result.substring(0, 4).equalsIgnoreCase("true")) {
                ...
                List<MinBalanceChargesPostPojo> minBalanceChargesPostReport = remoteObject.minBalanceChargesPostReport(Float.parseFloat(trsNo), acctType, userName, currentDate1, orgnBrCode);
    ...
    public void calculateBtnAction() {
        try {
            ...
            double total = 0f;
            resultList = remoteObject.minBalanceChargesCaculate(acctType, ymd.format(fromDate), ymd.format(toDate), orgnBrCode, userName, currentDate);
            if (resultList.isEmpty()) {
                setPostDisable(true);
                setCalDisable(true);
