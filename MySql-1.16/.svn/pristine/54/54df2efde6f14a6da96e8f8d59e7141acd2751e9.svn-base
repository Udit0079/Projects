...
//                query = "select ref_code,ref_desc from cbs_ref_rec_type where ref_rec_no='"+refRecNo+"' order by order_by";
    ...
    public List getDisbursementDetail(String acno) throws ApplicationException {
        List appLoanDetails = new ArrayList();
        try {
            Query resultlist = em.createNativeQuery("select sno,cast(amtdisbursed as decimal(25,2)),date_format(disbursementdt,'%d/%m/%Y') from loandisbursement  where acno = '" + acno + "'");
            appLoanDetails = resultlist.getResultList();
...
//                    + "and date_format(PlanAbortDt,'%Y%m%d') = (select date_format(max(PlanAbortDt),'%Y%m%d') from emirescheduled where acno = '" + acno + "' and PlanAbortDt is not null) ").getResultList();
            list = em.createNativeQuery("select b.dueDt,a.outbal from "
                    + "(select date_format(StartDueDt,'%d/%m/%Y') as dueDt,TotInstallAmt as outbal from emirescheduled where acno = '" + acno + "' "
                    + "and date_format(PlanAbortDt,'%Y%m%d') =  "
                    + "(select date_format(max(PlanAbortDt),'%Y%m%d') from emirescheduled where acno = '" + acno + "' and PlanAbortDt is not null))a, "
                    + "(select date_format(min(DUEDT),'%d/%m/%Y') as dueDt from emidetails where acno = '" + acno + "' and STATUS = 'Unpaid')b").getResultList();
