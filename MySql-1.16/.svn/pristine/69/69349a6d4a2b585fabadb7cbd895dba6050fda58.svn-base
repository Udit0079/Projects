 ...
 * @author saurabhsipl
 */
public class AdvncAgnstShareDebt extends BaseBean {
    ...
    public void viewPdf(){
        ...
        try {
            ...
            if (repOpt.equalsIgnoreCase("T")) {
                amtIn = "Amount in (Rs. in Thousand)";
                repOptIn = 1000;
            } else if (repOpt.equalsIgnoreCase("L")) {
                amtIn = "Amount in (Rs. in Lac)";
                repOptIn = 100000;
            } else if (repOpt.equalsIgnoreCase("C")) {
                amtIn = "Amount in (Rs. in Crore)";
                repOptIn = 10000000;
            ...
            String reptName ="";
            report = "Advance against Share and Debenture";            
            ...
            map.put("pReportIn", amtIn);
            map.put("SUBREPORT_DIR", getHttpSession().getServletContext().getRealPath("/WEB-INF/reports") + "/");
            List<AdvncAgnstShareDebtPojo> mainList = hoRemote.getAdvncAgnstShareAndDebt(reptName,new BigDecimal(repOptIn),getOrgnBrCode(),asOnDate);
            
            new ReportBean().openPdf("Advance Against Share & DXebentures_"+ymd.format(sdf.parse(asOnDate)), "AdvncAgnstShareDebt", new JRBeanCollectionDataSource(mainList), map);
