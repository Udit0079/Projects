 ...
 * @author Sudhir
 */
public class OnUs extends BaseBean {
    ...
    public void printReport() {
        try {
            ...
            onUsReportrDetails = otherFacadeRemote.getOnUsReport(ymdFormatter.format(getTrandate()), tranType, getOrgnBrCode(), fromTimeGenerate, toTimeGenerate, timeAllowed);
            ...
            if (transactionType.equals("1")) {
                new ReportBean().jasperReport("OnUsCashReport", "text/html", new JRBeanCollectionDataSource(onUsReportrDetails), fillParams, "On-Us Report");
                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
            } else {
                new ReportBean().jasperReport("OnUsTransferReport", "text/html", new JRBeanCollectionDataSource(onUsReportrDetails), fillParams, "On-Us Report");
                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
            }
        } catch (ApplicationException e) {
            this.setMessage(e.getExceptionCode().getExceptionMessage());
    ...
    public void pdfDownLoad() {
        try {
            ...
            if (transactionType.equals("1")) {
                new ReportBean().openPdf("On-Us Report_" + ymdFormatter.format(dmyFormatter.parse(getTodayDate())), "OnUsCashReport", new JRBeanCollectionDataSource(onUsReportrDetails), fillParams);
                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
            } else {
                new ReportBean().openPdf("On-Us Report_" + ymdFormatter.format(dmyFormatter.parse(getTodayDate())), "OnUsTransferReport", new JRBeanCollectionDataSource(onUsReportrDetails), fillParams);
                ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
            }

            HttpSession httpSession = getHttpSession();
            httpSession.setAttribute("ReportName", "On-Us Report");
    ...
    public String validations() {
        try {
            ...
            return "Error!!! Error occurred while validating field";
        }
        return "true";
    }
