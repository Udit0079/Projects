    ...
    public String yesButtonRegisterClose(String emFlag, String dtRegisterClose, String user,
        ...
        try {
            ...
            if (!dtRegisterClose.equalsIgnoreCase(Tempbd)) {
                throw new ApplicationException("Only Today Clearing Register Can be Closed.");
            }
            ...
            if (NRegDtss.isEmpty()) {
                throw new ApplicationException("No Record Exist for Closing");
            }
            if ((emFlag.equals("A")) || (emFlag.equals("B")) || (emFlag.equals("G"))) {
            ...
            if (txnStatus.isEmpty()) {
                ...
                int varA = updateQuery.executeUpdate();
                ...
                int varB = insertQuery.executeUpdate();
                ...
                int varC = deleteQuery.executeUpdate();
            ...
            if (status.isEmpty()) {
                throw new ApplicationException("No Register is Active in this CircleType to Close.");
            } //else {
            if ((emFlag.equals("A")) || (emFlag.equals("B")) || (emFlag.equals("G"))) {
            ...
            if (!txnStatuss.isEmpty()) {
                throw new ApplicationException("This Register contains Unverified Instruments. So Register Can't be closed.");
            }
            if (txnSts.equalsIgnoreCase("CLOSE")) {
                throw new ApplicationException("This Register has already been Closed.");
                ...
                int varD = updateQuery.executeUpdate();
            }
            // }
            ...
            if (var <= 0) {
                throw new ApplicationException("Problem in register closing");
            }
            
            if (ftsPostingRemote.getCodeForReportName("CLG-CONTRA-VOUCHER") == 1) {
                List totAmtList;
                double totAmt = 0;
                if ((emFlag.equals("A")) || (emFlag.equals("B")) || (emFlag.equals("G"))) {
                    totAmtList = em.createNativeQuery("select sum(amt) from ("
                            + "Select sum(TxnInstAmt) as amt From clg_ow_entry where txndate='" + dtRegisterClose + "' and orgnBrcode='" + brCode
                            + "' and emflag ='" + emFlag + "' and upper(TxnStatus)='V' "
                            + "UNION "
                            + "Select sum(TxnInstAmt) as amt From  stud_clg_ow_entry st,accountmaster am where txndate='" + dtRegisterClose + "' and emflag ='" + emFlag
                            + "' and st.Acno = am.ACNo and am.CurBrCode = '" + brCode + "' and upper(TxnStatus)='V') a").getResultList();

                } else {
                    totAmtList = em.createNativeQuery("select sum(amt) from ("
                            + "Select sum(TxnInstAmt) as amt from clg_localchq where txndate='" + dtRegisterClose + "' and orgnBrcode='" + brCode
                            + "' and emflag ='" + emFlag + "' and upper(TxnStatus)='V' "
                            + "UNION "
                            + "Select sum(TxnInstAmt) as amt From stud_clg_localchq st,accountmaster am where txndate='" + dtRegisterClose + "' and st.Acno = am.ACNo and "
                            + "am.CurBrCode = '" + brCode + "' and emflag ='" + emFlag + "' and upper(TxnStatus)='V') a").getResultList();
                }
                if (totAmtList.isEmpty()) {
                    throw new ApplicationException("Data does not exist for the Register");
                }
                Vector totAmtVec = (Vector) totAmtList.get(0);
                totAmt = Double.parseDouble(totAmtVec.get(0).toString());
                if (totAmt > 0) {

                    String crGlHead = brCode + ftsPostingRemote.getCodeFromCbsParameterInfo("CLG-CONTRA-CR-HEAD");
                    String drGlHead = brCode + ftsPostingRemote.getCodeFromCbsParameterInfo("CLG-CONTRA-DR-HEAD");

                    float trsNo = ftsPostingRemote.getTrsNo();

                    String rs = ftsPostingRemote.insertRecons("PO", crGlHead, 0, totAmt, Tempbd, Tempbd, 2, "Chqs.Sent For Coll.(E)", user, trsNo, null, 0f,
                    ...
                    rs = ftsPostingRemote.insertRecons("PO", drGlHead, 1, totAmt, Tempbd, Tempbd, 2, "Chqs.Sent For Coll(E)", user, trsNo, null, 0f,
                            "Y", "System", 0, 3, "", null, null, null, "A", 0, null, 0f, null, null, brCode, brCode, 0, null, null, null);
                    if (!rs.equalsIgnoreCase("True")) {
                        throw new ApplicationException(rs);
                    }
            ...
            if ((emFlag.equals("A")) || (emFlag.equals("B")) || (emFlag.equals("G"))) {
                ...
                int varB = deleteQuery.executeUpdate();
                ...
                int varC = insertQuery1.executeUpdate();
                ...
                int varD = deleteQuery1.executeUpdate();
            }
            ut.commit();
            return "Register Closed Successfully.";

        } catch (ApplicationException | IllegalStateException | NumberFormatException | SecurityException | HeuristicMixedException | HeuristicRollbackException | NotSupportedException | RollbackException | SystemException e) {
            try {
                ut.rollback();
                throw new ApplicationException(e);
