...
public class CentralizedLoanInterestCal extends BaseBean {
    ...
    public CentralizedLoanInterestCal() {
        try {
    ...
    public void centralizedLoanInterestCalculation() {
        String tmpMonth;
        this.setErrorMessage("");
        this.setReportFlag(false);
        this.setMessage("");
        ...
        try {
            ...
            resultList = centralizedLoanInterestCalMainRemote.centralizedLoanInterestCalculationCover(acType, this.acNature, fromDate, toDate,
                    getUserName(), branchCode);
            ...
            if (resultList.isEmpty()) {
                ...
                for (int i = 0; i < resultList.size(); i++) {
                    ...
                    pojo.setBranch(lt.getBranch());
                    pojo.setAcType(lt.getAcType());
                    pojo.setCustName(lt.getCustName());
                    pojo.setFromDate(lt.getFirstDt());
                    pojo.setToDate(lt.getLastDt());
                    mt.setAcNo(lt.getAcNo());
                    mt.setAcType(lt.getAcType());
                    mt.setBranch(lt.getBranch());
                    mt.setCustName(lt.getCustName());
                    mt.setFirstDt(lt.getFirstDt());
                    mt.setLastDt(lt.getLastDt());
                    double closingBal = lt.getClosingBal();
                    double Product = lt.getProduct();
                    ...
                    mt.setProduct(new BigDecimal(formatter.format(Product)));
                    pojo.setProduct(new BigDecimal(formatter.format(Product)));
                    mt.setRoi(new BigDecimal(formatter.format(lt.getRoi())));

                    pojo.setRoi(new BigDecimal(formatter.format(lt.getRoi())));
                    mt.setTotalInt(new BigDecimal(formatter.format(Math.round(acctInt))));
                    pojo.setInterestAmt(new BigDecimal(formatter.format(Math.round(acctInt))));
                    amount = amount + Math.round(acctInt);

                    mt.setIntTableCode(lt.getIntTableCode());
                    mt.setAccStatus(lt.getAccStatus());
                    mt.setAcNature(lt.getAcNature());
    ...
    public void Post() {
        ...
        try {
            ...
            String toDate = tmpDate.substring(6) + tmpDate.substring(3, 5) + tmpDate.substring(0, 2);
            setDisablePost(true);
            String msg = centralizedLoanInterestCalMainRemote.centralizedLoanInterestPosting(intCalc, acType, this.acNature, fromDate, toDate,
    ...
    public void Report(List<InterestCalculationPojo> resultList) {
        try {
            
            ComparatorChain chOb = new ComparatorChain();
            chOb.addComparator(new SortedByAcType());
            Collections.sort(resultList, chOb);
            ...
            List bankDetailsList = RemoteCode.getBranchNameandAddress(getOrgnBrCode());
            if (bankDetailsList.size() > 0) {
                ...
                new ReportBean().jasperReport("LOAN_INT_CAL_REPORT_CENTRALIZED", "text/html", new JRBeanCollectionDataSource(resultList), fillParams, "Centralized Loan Interest Cal");
