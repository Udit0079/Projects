...
//                            + "sh.custid='" + paramValue + "' ";
                    qry = "SELECT CCMD.customerid,CI.acno,CCMD.custname,DateOfBirth,CONCAT (fathername,' ',FatherMiddleName,' ',FatherLastName) as fatherName,mothername,mobilenumber,PAN_GIRNumber,"
                            ...
                            + " CCMD.customerid in(" + paramValue + ")"
                            + " union "
                            + " select a.cid,a.acno,a.cname,a.dob,a.fatherName,a.motherName,a.mob,a.pan,a.padd,a.madd,a.middle_name,a.last_name,"
                            + " IF(cs.status='C' ,'CLOSED',if( cs.status='A','ACTIVE','')) as status "
                            + " from (SELECT cc.customerid as cid,sh.Regfoliono as acno,cc.custname as cname,DateOfBirth as dob,CONCAT(cc.fathername,' ',cc.FatherMiddleName,' ',cc.FatherLastName) as fatherName,"
                            + " CONCAT (mothername,' ',MotherMiddleName,' ',MotherLastName) as motherName,mobilenumber as mob, PAN_GIRNumber as pan,PerAddressLine1 as padd,MailAddressLine1 as madd,"
                            + " ifnull(middle_name,'') as middle_name,ifnull(last_name,'') as last_name from cbs_customer_master_detail cc,share_holder sh where "
                            + " cc.customerid in (" + paramValue + ")"
                            + " and cc.customerid = sh.custid group by cc.customerid,sh.Regfoliono) a left join share_capital_issue sc on a.acno = sc.foliono "
                            + " left join certificate_share cs on sc.sharecertno =cs.certificateno where "+ statusCondQueryShare +" group by a.cid,a.acno";
