...
//                            }
    ...
    public List<LoanIntCertPojo> cbsrepLoanIntCert(String acno, String fromdt, String todt, String brncode) throws ApplicationException {
        ...
        try {
            ...
            tempList = em.createNativeQuery("SELECT a.acno,ifnull(c.title,''),a.CUSTNAME,c.CrAddress FROM accountmaster a,customermaster c WHERE a.ACNO='" + acno + "'  AND SUBSTRING(a.acno,5,6)=c.CustNo AND c.brncode = a.curbrcode AND SUBSTRING(a.acno,3,2)=c.actype  AND a.accttype  IN( select acctcode from accounttypemaster where acctnature in ('" + CbsConstant.TERM_LOAN + "','" + CbsConstant.DEMAND_LOAN + "','" + CbsConstant.CURRENT_AC + "')) AND a.curbrcode='" + brncode + "'").getResultList();
            if (!tempList.isEmpty()) {
...
//                    mailBlock = vector1.get(5).toString();
                ...
                if (acNature.equalsIgnoreCase(CbsConstant.CURRENT_AC)) {
                    tempList = em.createNativeQuery("select ifnull(sum(ifnull(dramt,0)),0) from ca_recon where ty=1 and trandesc in(3,4) and acno='" + acno + "' and DATE_FORMAT(dt,'%Y%m%d') between '" + fromdt + "' and '" + todt + "'").getResultList();
                } else {
                ...
                if (acNature.equalsIgnoreCase(CbsConstant.CURRENT_AC)) {
                    tempList = em.createNativeQuery("select ifnull(sum(ifnull(cramt,0)),0) from ca_recon where ty=0 and acno='" + acno + "'and DATE_FORMAT(dt,'%Y%m%d') between '" + fromdt + "' and '" + todt + "'").getResultList();
                } else {
                ...
                if (acNature.equalsIgnoreCase(CbsConstant.CURRENT_AC)) {
                    tempList = em.createNativeQuery("select cast(ifnull(sum(ifnull(dramt,0)),0) as decimal(25,2)) from ca_recon where ty=1 and trandesc <> 6 and acno='" + acno + "'and DATE_FORMAT(dt,'%Y%m%d') between '" + fromdt + "' and '" + todt + "'").getResultList();
                } else {
                    tempList = em.createNativeQuery("select cast(ifnull(sum(ifnull(dramt,0)),0) as decimal(25,2)) from loan_recon where ty=1 and trandesc <> 6 and acno='" + acno + "'and DATE_FORMAT(dt,'%Y%m%d') between '" + fromdt + "' and '" + todt + "'").getResultList();
                }
                ...
                double actualPrinAmt = prinamt - charge;
                if (acNature.equalsIgnoreCase(CbsConstant.CURRENT_AC)) {
                    if (actualPrinAmt < 0) {
                        actualPrinAmt = 0;
                    }
                ...
                pojo.setPrinAmt(new BigDecimal(actualPrinAmt));
...
//                    }
    ...
    public List<StockStatementRepPojo> getStockStatementDetails(String brcode, String acnochoice, String acno, String frdt, String todt) throws ApplicationException {
        ...
        List l = em.createNativeQuery("select Acno as Acno , securityChg as securityChg ,securityoption as securityoption , Particulars as Particulars, cast(lienvalue as Decimal) as lienvalue,"
                ...
                + "from  loan_stockstm where Entrydate between '" + frdt + "' and '" + todt + "'" + brcodeQuery + "" + acnochoicequery + " "
                + "union all "
                + "select  Acno as Acno , securityChg as securityChg ,securityoption as securityoption , Particulars as Particulars, cast(lienvalue as Decimal) as lienvalue, "
                + "Status as Status ,ifnull(Date_format(ReceivedSTMDt,'%d/%m/%Y'),'') as ReceivedSTMDt, "
                + "ifnull(Date_format(ExpiryDate,'%d/%m/%Y') ,'') as ExpiryDate,Date_format(Entrydate,'%d/%m/%Y') as Entrydate,Enteredby as Enteredby  "
                + "from loansecurity where substring(acno,3,2) in(select acctcode from accounttypemaster where acctnature = 'TL') "
                + "and Entrydate between '" + frdt + "' and '" + todt + "' and SecurityOption = 'STOCK' " + brcodeQuery + "" + acnochoicequery + "").getResultList();
