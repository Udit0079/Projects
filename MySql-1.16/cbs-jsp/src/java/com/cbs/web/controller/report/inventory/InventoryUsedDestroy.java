 ...
 * @author Ankit Verma
 */
public class InventoryUsedDestroy extends BaseBean {
    ...
    public InventoryUsedDestroy() {
        try {
            ...
            typeList.add(new SelectItem("U", "Used"));
            typeList.add(new SelectItem("D", "Destroyed"));
    ...
    public void viewReport() {
        ...
        if (msg.equalsIgnoreCase("ok")) {
            try {
                ...
                List<InventoryReportPojo> reportList = facadeRemote.getInvtUsedDestroyReport(type, dd3, branch);
                ...
                if (vec.size() > 0) {
                    fillParams.put("pReportName", "Used/Destroyed Inventory");
                ...
                if (reportList.size() > 0) {
                    rb.jasperReport("InventoryUsedDestroyed", "text/html", new JRBeanCollectionDataSource(reportList), fillParams, "Used/Destroyed Inventory");
    ...
    public void pdfDownLoad() {
        ...
        if (msg.equalsIgnoreCase("ok")) {
            try {
                String dt1[] = date.split("/");
                String dd3 = dt1[2] + dt1[1] + dt1[0];
                List<InventoryReportPojo> reportList = facadeRemote.getInvtUsedDestroyReport(type, dd3, brnCode);
                Map fillParams = new HashMap();
                CommonReportMethodsRemote beanLocal = (CommonReportMethodsRemote) ServiceLocator.getInstance().lookup("CommonReportMethods");
                List vec = beanLocal.getBranchNameandAddress(brnCode);
                if (vec.size() > 0) {
                ...
                ReportBean rb = new ReportBean();
                if (reportList.size() > 0) {
                    rb.openPdf("Used Destroyed Inventory_" + ymd.format(sdf.parse(getTodayDate())), "InventoryUsedDestroyed", new JRBeanCollectionDataSource(reportList), fillParams);
                    ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
                    HttpSession httpSession = getHttpSession();
                    httpSession.setAttribute("ReportName", "Used/Destroyed Inventory");
                } else {
                    setMsg("No detail exists");
                }
            } catch (Exception e) {
