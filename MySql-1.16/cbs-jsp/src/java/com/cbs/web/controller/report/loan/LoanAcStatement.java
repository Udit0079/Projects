...
//    }
    ...
    public String downLoadExcel() throws ApplicationException {
        ...
        try {
            ...
            if (npaStatus.equalsIgnoreCase("STANDARD") || npaStatus.equalsIgnoreCase("OPERATIVE")) {
                if (!resultList.isEmpty()) {
                    new ReportBean().dynamicExcelJasper("LoanAccountStatementExcel", "excel", new JRBeanCollectionDataSource(resultList), fillParams, report);
                } else {
                    errorMsg = "Data does not exist";
                }
            } else {
                if (!resultList.isEmpty()) {
                    ...
                    double totaOsBal = tDr - tCr;
                    fillParams.put("pTotalOsBal", new BigDecimal(totaOsBal));
                    new ReportBean().dynamicExcelJasper("LoanNpaAccountStatementExcel", "excel", new JRBeanCollectionDataSource(finalList), fillParams, report);
                } else {
                    errorMsg = "Data does not exist";
                }
            }
