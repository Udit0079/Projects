 ...
 * @author admin
 */
public class DividendReport extends BaseBean implements Serializable {

    private String errorMsg, foliono, folionoShow, acNoLen;
    ...
    /**
     * Creates a new instance of DevidentReport
     */
    public DividendReport() {
        try {
    ...
    public String viewReport() {
        try {
            if (validate()) {
                try {
                    List<ShareDividendPojo> resultList = horfr.getDividenDreport(ymd.format(frDt), ymd.format(toDt), foliono);
                    if (!resultList.isEmpty()) {
                        ...
                        fillParams.put("pReportName", "Share Dividend Report");
                        fillParams.put("pBankName", s[0]);
                        fillParams.put("pBankAddress", s[1]);
                        new ReportBean().jasperReport("ShareDividendReport", "text/html", new JRBeanCollectionDataSource(resultList), fillParams, "Share Dividend Report");
                        return "report";
                    } else {
            ...
            if (validate()) {
                try {
                    ...
                    if (!resultList.isEmpty()) {
                        ...
                        fillParams.put("pBankName", s[0]);
                        fillParams.put("pBankAddress", s[1]);
                        new ReportBean().openPdf("Share Dividend Report", "ShareDividendReport", new JRBeanCollectionDataSource(resultList), fillParams);
                        ((HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse()).sendRedirect("/cbs-jsp/faces/report/ReportPage.jsp");
                        HttpSession httpSession = getHttpSession();
                        httpSession.setAttribute("ReportName", "Share Dividend Report");
                    } else {
                        setErrorMsg("Data does not exist !");
                    }
                } catch (Exception e) {
    ...
    public boolean validate() {
        if ((foliono == null) || (foliono.equalsIgnoreCase(""))) {
            ...
            return false;
        } else if (!this.foliono.equalsIgnoreCase("") && ((this.foliono.length() != 12) && (this.foliono.length() != (Integer.parseInt(this.getAcNoLen()))))) {
            errorMsg = "Please enter valid folio no";
            return false;
        } else if (frDt == null) {
    ...
    public void folioFromMethod() {
        ...
        if (!this.folionoShow.equalsIgnoreCase("") && ((this.folionoShow.length() != 12) && (this.folionoShow.length() != (Integer.parseInt(this.getAcNoLen()))))) {
            setErrorMsg("Please enter Proper Folio Number");
            return;
        }
        try {
